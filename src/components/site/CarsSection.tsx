    ...
    <section id="voitures" className="py-20 bg-secondary/30">
      <div className="container">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold">Notre flotte</h2>
          <p className="mt-3 text-muted-foreground">Des voitures fiables a des prix imbattables</p>
        </div>
            ...
            : cars?.map((car) => (
                <Card key={car.id} className="overflow-hidden shadow-card hover:shadow-elegant transition-all hover:-translate-y-1 duration-300 border-border/60">
                  <div className="aspect-[4/3] bg-secondary overflow-hidden">
                    <img
                      src={car.image || fallbackImage(car.name)}
                      alt={car.name}
                      loading="lazy"
                      width={800}
                      height={600}
                  ...
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="text-primary font-bold whitespace-nowrap">
                        {Number(car.price)} DH<span className="text-xs text-muted-foreground font-normal"> / jour</span>
                      </span>
                    </div>
                    <Button asChild className="w-full">
                      <a
                        href={waLink(`Bonjour Atlas Cars, je souhaite reserver la ${car.name} (${car.price} DH/jour).`)}
                        target="_blank"
                        rel="noopener"
                      >
