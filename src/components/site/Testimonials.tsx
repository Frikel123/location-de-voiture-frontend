      ...
      <div className="container">
        <div className="mb-12 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div className="max-w-2xl">
            <p className="mb-3 text-sm font-semibold uppercase tracking-[0.22em] text-primary">{isFr ? "Avis clients" : "Customer reviews"}</p>
            <h2 className="text-balance text-3xl font-bold tracking-tight md:text-5xl">{isFr ? "Une location qui inspire confiance." : "Rental that feels trustworthy."}</h2>
          </div>
          <div className="flex items-center gap-1 rounded-full border border-border bg-card px-4 py-2 shadow-card">
            {Array.from({ length: 5 }).map((_, index) => <Star key={index} className="h-4 w-4 fill-primary text-primary" />)}
          ...
          <CarouselContent>
            {reviews.map((review, index) => (
              <CarouselItem key={review.name} className="md:basis-1/2 lg:basis-1/3">
                <motion.article initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.25 }} transition={{ delay: index * 0.06 }} className="h-full rounded-3xl border border-border/70 bg-card p-6 shadow-card">
                  <div className="mb-5 flex items-center gap-1">
                    {Array.from({ length: 5 }).map((_, star) => <Star key={star} className="h-4 w-4 fill-primary text-primary" />)}
                  </div>
                  <p className="min-h-28 text-base leading-7 text-muted-foreground">"{review.text}"</p>
                  <div className="mt-6 flex items-center gap-3">
                    ...
                    <div><p className="font-semibold">{review.name}</p><p className="text-sm text-muted-foreground">{review.route}</p></div>
                  </div>
                </motion.article>
              </CarouselItem>
            ))}
